...
export default function InquiryForm() {
  ...
  return <form className="inquiry" onSubmit={submit}>
    <label>Name<input name="name" placeholder="Your name" maxLength={120} required /></label>
    <label>Email<input type="email" name="email" placeholder="you@company.com" maxLength={254} required /></label>
    <label>What do you need?<select name="service" defaultValue="" required><option value="" disabled>Select a service</option><option>Custom business website</option><option>Full-stack web application</option><option>Website redesign</option><option>Not sure yet</option></select></label>
    <label>Tell me about the project<textarea name="project" placeholder="Goals, features, timeline, budget..." rows={4} minLength={10} maxLength={5000} required /></label>
